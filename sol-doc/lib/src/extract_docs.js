    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const [importFile, fileOutput] of Object.entries(output.contracts)) {
            ...
            for (const contractOutput of Object.values(fileOutput)) {
                ...
                const content = yield (0, util_1.promisify)(fs.readFile)(filePath, { encoding: 'utf-8' });
...
function extractDocsFromFile(ast, source) {
    ...
    const visit = (node, currentContractName) => {
        const { offset } = (0, sol_ast_1.splitAstNodeSrc)(node.src);
        if ((0, sol_ast_1.isSourceUnitNode)(node)) {
            for (const child of node.nodes) {
                visit(child);
            }
        }
        else if ((0, sol_ast_1.isContractDefinitionNode)(node)) {
        ...
        else if ((0, sol_ast_1.isVariableDeclarationNode)(node)) {
        ...
        else if ((0, sol_ast_1.isFunctionDefinitionNode)(node)) {
            const natspec = getNatspecBefore(source.content, offset);
            docs.contracts[currentContractName].methods.push({
                file: source.path,
                line: getAstNodeLineNumber(node, source.content),
        ...
        else if ((0, sol_ast_1.isStructDefinitionNode)(node)) {
            const natspec = getNatspecBefore(source.content, offset);
        ...
        else if ((0, sol_ast_1.isEnumDefinitionNode)(node)) {
            const natspec = getNatspecBefore(source.content, offset);
        ...
        else if ((0, sol_ast_1.isEventDefinitionNode)(node)) {
            const natspec = getNatspecBefore(source.content, offset);
...
function extractAcessorParameterDocs(typeNameNode, natspec, source) {
    ...
    if ((0, sol_ast_1.isMappingTypeNameNode)(typeNameNode)) {
        // Handle mappings.
        ...
        do {
            ...
            node = node.valueType;
        } while ((0, sol_ast_1.isMappingTypeNameNode)(node));
    }
    else if ((0, sol_ast_1.isArrayTypeNameNode)(typeNameNode)) {
        // Handle arrays.
        let node = typeNameNode;
        let order = 0;
        do {
            ...
            node = node.baseType;
        } while ((0, sol_ast_1.isArrayTypeNameNode)(node));
    }
    return params;
}
function extractAccesorReturnDocs(typeNameNode, natspec, source) {
    ...
    if ((0, sol_ast_1.isMappingTypeNameNode)(typeNameNode)) {
        // Handle mappings.
        let node = typeNameNode;
        while ((0, sol_ast_1.isMappingTypeNameNode)(node.valueType)) {
            node = node.valueType;
        }
    ...
    else if ((0, sol_ast_1.isUserDefinedTypeNameNode)(typeNameNode)) {
...
function extractStructFieldDocs(fieldNodes, natspec, source) {
    ...
    for (const field of fieldNodes) {
        const { offset } = (0, sol_ast_1.splitAstNodeSrc)(field.src);
        fields[field.name] = {
            file: source.path,
...
function extractEnumValueDocs(valuesNodes, natspec, source) {
    ...
    for (const value of valuesNodes) {
        const { offset } = (0, sol_ast_1.splitAstNodeSrc)(value.src);
        values[value.name] = {
            file: source.path,
...
function getAstNodeLineNumber(node, code) {
    return offsetToLineIndex(code, (0, sol_ast_1.splitAstNodeSrc)(node.src).offset) + 1;
}
