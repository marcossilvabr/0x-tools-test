...
describe('generateMarkdownFromDocs()', () => {
    const URL_PREFIX = (0, random_docs_1.randomWord)();
    const DOCS = {
        contracts: Object.assign({}, _.mapValues(_.groupBy(_.times(_.random(2, 8), () => ((name) => (Object.assign({ name }, (0, random_docs_1.randomContract)(name))))(`${(0, random_docs_1.randomWord)()}Contract`)), 'name'), g => g[0])),
    };
    let md;
    let mdLines;
    ...
    before(() => {
        md = (0, gen_md_1.generateMarkdownFromDocs)(DOCS, { urlPrefix: URL_PREFIX });
        mdLines = md.split('\n');
    });
    it('generates entries for all contracts', () => {
        ...
        for (const [contractName, contract] of Object.entries(DOCS.contracts)) {
            (0, chai_1.expect)(headers).to.include(`${contract.kind} \`${contractName}\``);
        }
    });
    it('generates entries for all enums', () => {
        ...
        for (const contract of Object.values(DOCS.contracts)) {
            for (const enumName of Object.keys(contract.enums)) {
                (0, chai_1.expect)(headers).to.include(`\`${enumName}\``);
            }
        }
    });
    it('generates entries for all structs', () => {
        ...
        for (const contract of Object.values(DOCS.contracts)) {
            for (const structName of Object.keys(contract.structs)) {
                (0, chai_1.expect)(headers).to.include(`\`${structName}\``);
            }
        }
    });
    it('generates entries for all events', () => {
        ...
        for (const contract of Object.values(DOCS.contracts)) {
            for (const event of contract.events) {
                (0, chai_1.expect)(headers).to.include(`\`${event.name}\``);
            }
        }
    });
        ...
        for (const contract of Object.values(DOCS.contracts)) {
            for (const method of contract.methods) {
                if (method.kind === extract_docs_1.FunctionKind.Fallback) {
                    (0, chai_1.expect)(headers).to.include(`\`<fallback>\``);
                }
                else if (method.kind === extract_docs_1.FunctionKind.Constructor) {
                    (0, chai_1.expect)(headers).to.include(`\`constructor\``);
                }
                else {
                    (0, chai_1.expect)(headers).to.include(`\`${method.name}\``);
                }
            }
        }
    });
    it('prefixes all URLs with the prefix', () => {
        ...
        for (const url of urls) {
            (0, chai_1.expect)(url.startsWith(URL_PREFIX)).to.be.true();
        }
    });
});
