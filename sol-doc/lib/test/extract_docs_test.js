...
describe('extractDocsAsync()', () => {
    ...
    before(() => __awaiter(void 0, void 0, void 0, function* () {
        docs = yield (0, extract_docs_1.extractDocsAsync)(_.shuffle(INPUT_FILE_PATHS));
    }));
    describe('contracts', () => {
        it('extracts all contracts with docs', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            for (const contract of INPUT_CONTRACTS) {
                ...
                (0, chai_1.expect)(cd).to.exist('');
                if (NO_DOCS.includes(contract)) {
                    (0, chai_1.expect)(cd.doc).to.eq('');
                }
                else {
                    (0, chai_1.expect)(cd.doc).to.eq(createDocString(contract));
                }
                (0, chai_1.expect)(cd.line, `${contract}.line`).to.eq(contractLines[contract]);
            }
        }));
        it('extracts contract inheritance', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            for (const contract of INPUT_CONTRACTS) {
                const cd = docs.contracts[contract];
                (0, chai_1.expect)(cd.inherits).to.deep.eq(contractInherits[contract]);
            }
        }));
    });
    describe('methods', () => {
        function assertMethodDocs(fullMethodName, props) {
            ...
            const m = docs.contracts[contractName].methods.find(_m => _m.name === methodName);
            {
                ...
                (0, chai_1.expect)(m).to.exist('');
                (0, chai_1.expect)(m.visibility).to.eq(props.visibility);
                (0, chai_1.expect)(m.contract).to.eq(contractName);
                (0, chai_1.expect)(m.doc).to.eq(doc);
            }
            const params = props.params || {};
            (0, chai_1.expect)(Object.keys(m.parameters), 'number of parameters').to.be.length(Object.keys(params).length);
            for (const [paramName, paramDoc] of Object.entries(params)) {
                ...
                (0, chai_1.expect)(actualParam).to.exist('');
                (0, chai_1.expect)(actualParam.doc).to.eq(doc);
                (0, chai_1.expect)(actualParam.line).to.eq(paramDoc.line);
                (0, chai_1.expect)(actualParam.storageLocation).to.eq(storage);
                (0, chai_1.expect)(actualParam.type).to.eq(paramDoc.type);
            }
            const returns = props.returns || {};
            (0, chai_1.expect)(Object.keys(m.returns), 'number of returns').to.be.length(Object.keys(returns).length);
            for (const [returnName, returnDoc] of Object.entries(returns)) {
                ...
                (0, chai_1.expect)(actualReturn).to.exist('');
                (0, chai_1.expect)(actualReturn.doc).to.eq(doc);
                (0, chai_1.expect)(actualReturn.line).to.eq(returnDoc.line);
                (0, chai_1.expect)(actualReturn.storageLocation).to.eq(storage);
                (0, chai_1.expect)(actualReturn.type).to.eq(returnDoc.type);
            }
        }
        ...
        describe('`BaseContract`', () => {
            ...
            it('`baseContractField3`', () => {
                ...
                (0, chai_1.expect)(docs.contracts.TestContract.events.find(e => e.name === 'baseContractField3')).to.eq(undefined);
            });
        });
    });
    describe('events', () => {
        function assertEventDocs(fullEventName, props) {
            ...
            const e = docs.contracts[contractName].events.find(_e => _e.name === eventName);
            {
                ...
                (0, chai_1.expect)(e.contract).to.eq(contractName);
                (0, chai_1.expect)(e.doc).to.eq(doc);
            }
            const params = props.params || {};
            (0, chai_1.expect)(Object.keys(e.parameters), 'number of parameters').to.be.length(Object.keys(params).length);
            for (const [paramName, paramDoc] of Object.entries(params)) {
                ...
                (0, chai_1.expect)(actualParam.doc).to.eq(doc);
                (0, chai_1.expect)(actualParam.line).to.eq(paramDoc.line);
                (0, chai_1.expect)(actualParam.indexed).to.eq(isIndexed);
                (0, chai_1.expect)(actualParam.type).to.eq(paramDoc.type);
            }
        }
        describe('`BaseContract`', () => {
    ...
    describe('enums', () => {
        function assertEnumDocs(fullEnumName, props) {
            ...
            (0, chai_1.expect)(Object.keys(e.values), 'number of values').to.be.length(Object.keys(values).length);
            for (const [valueName, valueDoc] of Object.entries(values)) {
                ...
                (0, chai_1.expect)(actualValue).to.exist('');
                (0, chai_1.expect)(actualValue.doc).to.eq(doc);
                (0, chai_1.expect)(actualValue.line).to.eq(valueDoc.line);
                (0, chai_1.expect)(actualValue.value).to.eq(valueDoc.value);
            }
        }
    ...
    describe('structs', () => {
        function assertStructDocs(fullStructName, props) {
            ...
            const s = docs.contracts[contractName].structs[`${contractName}.${structName}`];
            {
                ...
                (0, chai_1.expect)(s).to.exist('');
                (0, chai_1.expect)(s.contract).to.eq(contractName);
                (0, chai_1.expect)(s.doc).to.eq(doc);
            }
            const fields = props.fields || {};
            (0, chai_1.expect)(Object.keys(s.fields), 'number of fields').to.be.length(Object.keys(fields).length);
            for (const [fieldName, fieldDoc] of Object.entries(fields)) {
                ...
                (0, chai_1.expect)(actualField).to.exist('');
                (0, chai_1.expect)(actualField.doc).to.eq(doc);
                (0, chai_1.expect)(actualField.line).to.eq(fieldDoc.line);
                (0, chai_1.expect)(actualField.type).to.eq(fieldDoc.type);
                (0, chai_1.expect)(actualField.storageLocation).to.eq(extract_docs_1.StorageLocation.Default);
                (0, chai_1.expect)(actualField.indexed).to.eq(false);
            }
        }
        describe('`LibraryContract`', () => {
            it('`LibraryStruct`', () => {
